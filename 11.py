    ...
    def max_anti_diagonal_product():
        ...
        for i in range(len(array) - 3):
            for j in range(3, len(array[0])):
                ...
                for k in range(4):
                    product *= array[i + k][j - k]
                anti_diagonal_products.append(product)
        return max(anti_diagonal_products)
    
    return max(max_row_product(),max_column_product(),max_diagonal_product(),max_anti_diagonal_product())

print(calc_max_product_two_d_array(number))